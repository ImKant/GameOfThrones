...
with tf.name_scope('hidden_layer'): #隐层。将隐层权重、偏置、净输入放在一起
    with tf.name_scope('weight'): #权重
        ...
        tf.histogram_summary('hidden_layer/weight', W1)
    with tf.name_scope('bias'): #偏置
        b1 = tf.Variable(tf.zeros([1,10])+0.1)
        tf.histogram_summary('hidden_layer/bias', b1)
    with tf.name_scope('Wx_plus_b'): #净输入
        Wx_plus_b1 = tf.matmul(xs,W1) + b1
        tf.histogram_summary('hidden_layer/Wx_plus_b',Wx_plus_b1)
...
with tf.name_scope('output_layer'): #输出层。将输出层权重、偏置、净输入放在一起
    with tf.name_scope('weight'): #权重
        ...
        tf.histogram_summary('output_layer/weight', W2)
    with tf.name_scope('bias'): #偏置
        b2 = tf.Variable(tf.zeros([1,1])+0.1)
        tf.histogram_summary('output_layer/bias', b2)
    with tf.name_scope('Wx_plus_b'): #净输入
        Wx_plus_b2 = tf.matmul(output1,W2) + b2
        tf.histogram_summary('output_layer/Wx_plus_b',Wx_plus_b2)
output2 = Wx_plus_b2
...
sess = tf.Session()
sess.run(init)
merged = tf.merge_all_summaries() #将图形、训练过程等数据合并在一起
writer = tf.train.SummaryWriter('logs',sess.graph) #将训练日志写入到logs文件夹下
